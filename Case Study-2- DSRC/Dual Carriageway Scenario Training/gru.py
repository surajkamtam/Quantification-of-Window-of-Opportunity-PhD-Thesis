...
# ---------------------- Data Loader ----------------------
def load_sequences_from_csv(csv_file, look_back=10, look_forward=5):
...
# ---------------------- Training Pipeline ----------------------
def train_sequence_model(csv_path='sequence_data_with_jerk_70_noAttack.csv'):
    print("[INFO] Loading sequence data from CSV...")
    look_back = 10
    look_forward = 5
    ...
    # Callback to save weights every 25 epochs
    save_weights_callback = LambdaCallback(
        on_epoch_end=lambda epoch, logs: model.save_weights(f"gru_epoch_{epoch+1}.weights.h5") 
    ...
    print("[INFO] Saving visualizations and final model weights...")
    plot_sequence_predictions(model, X_test, y_test)
    plot_training_history(history)
    plot_confusion_matrix(y_test, model.predict(X_test))
    model.save_weights("gru.weights.h5")
    print("[INFO] Final model weights saved!")

    return model

# ---------------------- Main Entry ----------------------
if __name__ == "__main__":
    model = train_sequence_model('sequence_data_with_jerk_70_noAttack.csv')
