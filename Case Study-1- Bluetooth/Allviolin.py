...
combined_files_45 = ['combined_LOSAA.csv', 'combined_LOSAB.csv', 'combined_LOSAC.csv', 'combined_LOSAD.csv', 'combined_LOSAE.csv',
...
file_labels_45 = {'combined_LOSAA.csv': 'LOS AA', 'combined_LOSAB.csv': 'LOS AB', 'combined_LOSAC.csv': 'LOS AC', 'combined_LOSAD.csv': 'LOS AD', 'combined_LOSAE.csv': 'LOS AE',
...
# Load data for FFS 45 mph
all_dfs_45 = []
for file in combined_files_45:
    df = pd.read_csv(file)
    df['File'] = file_labels_45[file]
    df['FFS'] = '45 mph'
    all_dfs_45.append(df)

# Combine both dataframes
combined_df = pd.concat(all_dfs_60 + all_dfs_45)

g = sns.catplot(data=combined_df, y='AtK Msg Duration', hue='FFS', palette={'60 mph': 'green', '45 mph': 'blue'}, kind='violin', col='File', col_wrap=5, split=True, height=4, aspect=1.2)

# Create the legend manually on the LOS AE subplot
legend_ax = g.axes.flatten()[4]  # LOS AE subplot
legend_labels = ['60 mph', '45 mph']
...
g.set_titles('')
g.set_axis_labels('Level Of Service', 'Attacker Duration')
...
for ax, file_label in zip(g.axes.flatten(), combined_df['File'].unique()):
    ...
    above_threshold = combined_df[(combined_df['File'] == file_label) & (combined_df['AtK Msg Duration'] > 45)]['AtK Msg Duration'].count()
    below_threshold = combined_df[(combined_df['File'] == file_label) & (combined_df['AtK Msg Duration'] <= 45)]['AtK Msg Duration'].count()
