...
def change_vehs_per_hour(rou_xml_file, new_vehs_per_hour, vehicle_ids):
    tree = ET.parse(rou_xml_file)
    root = tree.getroot()

    for flow in root.findall('.//flow'):
        if flow.get('id') in vehicle_ids:
...
if __name__ == "__main__":
    ...
    vehs_per_hour_a = [400,500,700,900,1100]
    vehs_per_hour_b = [400,500,700,900,1100]
    ...
    for vehs_per_hour_a_val, vehs_per_hour_b_val in itertools.product(vehs_per_hour_a, vehs_per_hour_b):
        vehs_output_folder = os.path.join(output_base_folder, f"vehsPerHour_{vehs_per_hour_a_val}_{vehs_per_hour_b_val}")
        ...
        for depart_value in range(initial_depart_value, final_depart_value):  
            ...
            for i in range(1, simulation_runs + 1):  
                ...
                # Change vehsPerHour value for (veh0, veh1)
                change_vehs_per_hour(rou_xml_file_path, vehs_per_hour_a_val, ["veh0", "veh1"])
                
                # Change vehsPerHour value for (veh2, veh3)
                change_vehs_per_hour(rou_xml_file_path, vehs_per_hour_b_val, ["veh2", "veh3"])
