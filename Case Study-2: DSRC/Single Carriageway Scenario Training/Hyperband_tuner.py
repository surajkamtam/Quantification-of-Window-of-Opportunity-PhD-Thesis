...
class SequenceHyperModel(HyperModel):
    def __init__(self, input_shape, output_steps, n_features):
    ...
    def build(self, hp):
        ...
        x = Conv1D(
            filters=hp.Int('num_filters', 16, 64, step=16),
            kernel_size=3,
            activation='relu',
            padding='same')(inputs)
        ...
        x = Dense(self.output_steps * self.n_features)(x)
        outputs = Reshape((self.output_steps, self.n_features))(x)
...
def run_hyperband_tuning(X_train, y_train, X_val, y_val, input_shape, output_steps, n_features):
    hypermodel = SequenceHyperModel(input_shape, output_steps, n_features)
...
# ---------------------- Training Pipeline ----------------------
def train_sequence_model(csv_path='Noattack_sequence_data_with_jerk.csv'):
    ...
    model, tuner = run_hyperband_tuning(X_train, y_train, X_val, y_val, (look_back, n_features), look_forward, n_features)
    history = model.fit(
       X_train, y_train,
       validation_data=(X_val, y_val),
       epochs=10,
...
if __name__ == "__main__":
    model = train_sequence_model('Noattack_sequence_data_with_jerk.csv')
