...
int atkInt;
bool sporadictime= false;
std::random_device rd;
std::mt19937 generator(rd());
std::uniform_real_distribution<double> distribution(0.0, 1.0);
std::uniform_int_distribution<> intdist(10, 20);
std::uniform_int_distribution<> gapDist(5, 100);
std::uniform_int_distribution<> persistdist(930, 960);
double falseSpeed;
int attack;
simtime_t spStart;
int spCount=0;
int spDuration = 0;
int gapDuration = 0;             // gap duration (5-100 seconds)
simtime_t gapStart;
int persiststart=0;
...
void AttackApp::initialize(int stage) {
    ...
    if (stage == 1) {
        ...
        if(attack == 0){
            if (attackPolicy == 0 || attackPolicy == 1 || attackPolicy == 2) {
        ...
        if (attack == 1 || attack == 2) {
            ...
            if (attackPolicy == 0) {  // Persistent attack
                DoSAttack = true;
                persiststart= persistdist(generator);
...
void AttackApp::handleSelfMsg(cMessage *msg) {
    if (attackPolicy == 0) {  // Persistent attack: Always active
        ...
        if (!sporadictime && spCount < 2) {
            // Not attacking currently, check if in gap or ready to start attack
            if (spCount == 0) {
                // First attack: check random chance to start immediately
                double randomValue = distribution(generator);
                if (randomValue <= sP) {
                    sporadictime = true;
                    spStart = simTime();
                    spDuration = intdist(generator);   // Attack duration 10-20 seconds
                    DoSAttack = true;
                    spCount++;
                } else {
                    DoSAttack = false;
                ...
                // After first attack: check if gap is over before next attack
                if (simTime() - gapStart >= gapDuration) {
                    double randomValue = distribution(generator);
                    if (randomValue <= sP) {
                        sporadictime = true;
                        spStart = simTime();
                    ...
                    DoSAttack = false; // Still in gap waiting
                }
            }
        } else if (sporadictime) {
            // Currently in attack
            if (simTime() - spStart < spDuration) {
                DoSAttack = true;
            } else {
                // Attack finished, start gap timer
                sporadictime = false;
                gapStart = simTime();
                gapDuration = gapDist(generator);    // Gap duration 5-100 seconds
                DoSAttack = false;
            }
        } else {
            // All attacks done or waiting
            DoSAttack = false;
        }
    }
    ...
    if (simTime()>=persiststart && DoSAttack && simTime()<=persiststart+20) {
        if (attack == 1) {
...
//    }
    ...
    if (msg->isName("beacon")) {
        ...
        wsm->setAttackstart(persiststart);
        populateWSM(wsm);
        sendDown(wsm);
    }
